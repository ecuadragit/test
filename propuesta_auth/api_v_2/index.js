...
const bodyParser = require('body-parser'); // Agregué esta línea para importar bodyParser
...
app.use(bodyParser.json()); // Utiliza bodyParser para el análisis del cuerpo
...
app.post('/api/actualizar-monto-seguro', (req, res) => {
  ...
  db.query(query, (error, results) => {
    if (error) {
      ...
      res.status(400).json({ error: 'Error al actualizar monto_seguro' });
    } else {
...
app.post('/api/ejemplo', (req, res) => {
  const { dni, phone, carplate } = req.body;
  console.log("ksaudygksuyf");
  // Aquí podrías realizar cualquier lógica adicional según tus necesidades
  // Envia un mensaje de respuesta positiva
  res.json({ message: 'Respuesta positiva recibida', data: { dni, phone, carplate } });
});
